...
public class cokBoyutluOrnek2 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < notlar.length; i++) {
            if (notlar[i][0]<50){
                System.out.println((i+1) + ". Ogrencinin final notu :"+notlar[i][1]);
            }
        }
        //c. Ortalaması 70 den buyuk olanlar için ekrana “Geçti”, eşit ve ya kucuk olanlar için “Kaldı” yazdırınız.

        for (int i = 0; i < notlar.length; i++) {
            double ortalama = notlar[i][0]*0.4 + notlar[i][1]*0.6;
            if (ortalama>70){
                System.out.println((i+1) + ". ogrencinin ortalamasi : " + ortalama+"     Gecti");
            } else{
                System.out.println((i+1) + ". ogrencinin ortalamasi : " + ortalama+"     Kaldi");
            }
        }
        //d. Vizesi 60-70 arası olup Finali 70 den büyük olan öğrencilerin notlarını
        //   ekrana yazdırınız ve kaç adet olduğunu bulup ekrana yazdırınız.
        int sayac = 0;
        for (int i = 0; i < notlar.length; i++) {
            if (notlar[i][0]>60 && notlar[i][0]<=70 && notlar[i][1]>70){
                System.out.println((i+1) + ". Ogrencinin vize ve final notu :"+notlar[i][0]+"  "+notlar[i][1]);
                sayac+=1;
